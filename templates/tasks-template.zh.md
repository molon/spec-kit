...
- **用户故事 1（P1）**：可在基础（第 2 阶段）完成后开始 - 不依赖其他故事
- **用户故事 2（P2）**：可在基础（第 2 阶段）完成后开始 - 可能与 US1 集成但应可独立测试
- **用户故事 3（P3）**：可在基础（第 2 阶段）完成后开始 - 可能与 US1/US2 集成但应可独立测试

### 每个用户故事内部

- 测试（如果包含）必须先编写并确认失败后再实现
- 模型先于服务
- 服务先于端点
- 核心实现先于集成
- 故事完成后再进入下一个优先级
...
- 所有标记 [P] 的设置任务可以并行运行
- 所有标记 [P] 的基础任务可以并行运行（在第 2 阶段内）
- 基础阶段完成后，所有用户故事可以并行开始（如果团队容量允许）
- 一个用户故事中所有标记 [P] 的测试可以并行运行
- 一个故事中标记 [P] 的模型可以并行运行
- 不同用户故事可以由不同团队成员并行处理
